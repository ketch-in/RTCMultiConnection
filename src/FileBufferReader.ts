...
// --------------------------------------------------
export function FileBufferReader() {
  var fbr = this;
  ...
  function fbrClone(from?: any, to?: any) {
    ...
    for (var name in from) {
      to[name] =
        typeof to[name] == "undefined" ? fbrClone(from[name], null) : to[name];
    }
...
export function FileSelector() {
  ...
  function selectFile(callback?: any, multiple?: any, directory?: any) {
    ...
    file.onchange = function () {
      if (multiple) {
        ...
        var arr = [];
        Array.from(file.files).forEach(function (file: any) {
...
function FileBufferReceiver(fbr) {
  ...
  function receive(chunk?: any, callback?: any) {
    if (!chunk.uuid) {
    ...
    if (chunk.end) {
      ...
      var blob: any = new Blob(chunksArray, {
        type: chunk.type,
      });
...
var pack = function (serialized) {
  ...
  for (i = 0; i < serialized.length; i++) {
    ...
    if (type === Types.BUFFER) {
      ...
      view.setUint8(cursor, Types.BLOB);
    } else {
 ...
 */
var deserialize = function (buffer?: any, callback?: any) {
...
(<any>window).FileConverter = FileConverter;
(<any>window).FileSelector = FileSelector;
(<any>window).FileBufferReader = FileBufferReader;
